...
const float PSR_min_thresh = 3.0; //minimum PSR score for occlusion
...
{
    //int interpolation = cv::INTER_LINEAR; //caused jerky but accurate tracking
    int interpolation = cv::INTER_CUBIC; //causes smooth and accurate tracking
...
void STAPLE_TRACKER::getFeatureMap(cv::Mat &im_patch, const char *feature_type, cv::MatND &output)
{
    ...
    //cout << "\nfhog28 Mat dims: col, rows, channels " << output.cols << ", " << output.rows << ", " << output.channels() << endl;
#endif

   /* cv::Mat im_patch_hog;
    cvtColor(im_patch, im_patch_hog, cv::COLOR_BGR2GRAY);
    HOG* hogTemplate = new HOG(im_patch_hog, 4, 28);
    cv::MatND output_hog = hogTemplate->extract();
    cout << "\noutput_hog Mat dims: col, rows, channels " << output_hog.cols << ", " << output_hog.rows << ", " << output_hog.channels() << endl;*/
...
void STAPLE_TRACKER::tracker_staple_train(const cv::Mat &im, bool first)
{
    ...
    int dft_type = 0; // FFT Computation done through 1-CPU or 2-CUDA GPU or 3-CUDA GPU (CuFFT)
    ...
    if (dft_type == 0)
    {
        for (int i = 0; i < xt.channels(); i++)
        {
            cv::Mat dimf_cpu;
            tic = cv::getTickCount();
            cv::dft(xtsplit[i], dimf_cpu);
            toc = cv::getTickCount() - tic;
            //cout << "CPU DFT Computation Ticks " << toc << endl;
            xtf.push_back(dimf_cpu);
            //cout << "dimf_cpu " << dimf_cpu.size() << endl;
        }
    }
    ...
    for (int i = 0; i < 26 ; i++)
    {
        cv::matchTemplate(xtsplit[i], xtsplit[i+1], Template_match, cv::TM_CCORR_NORMED, cv::noArray());
        minMaxLoc(Template_match, &minVal, &maxVal, &minLoc, &maxLoc, cv::noArray());
    }
...
// TESTING step
cv::Rect STAPLE_TRACKER::tracker_staple_update(const cv::Mat &im, float* response_score_max, cv::Mat* response_matrix)
{
    ...
    //imshow("STAPLE CF filter", response_cf);
    ...
    //printf("[%s][%s][%4.4d][MaxVal %f ] \n", __FILE__, __func__, __LINE__, maxVal);


    *response_score_max = maxVal;
    *response_matrix = response;

    float centerx = (1 + norm_delta_area.width) / 2 - 1;
    float centery = (1 + norm_delta_area.height) / 2 - 1;

    float Compute_PSR = ComputePSR(response);
    //std::cout << "STAPLE PSR Score " << Compute_PSR << std::endl;

    if (Compute_PSR > PSR_min_thresh) //normal tracking in case PSR limit exceeds minimum
    {
    ...
    //std::cout << "Location coordinates" << location << std::endl;
...
float STAPLE_TRACKER::ComputePSR(const cv::Mat& correlation_mat)
{//Compute Peak-to-Sidelobe Ratio
    ...
    //Define PSR mask
    int win_size = floor(11);
    cv::Rect mini_roi = cv::Rect(std::max(max_loc.x - win_size, 0), std::max(max_loc.y - win_size, 0), 11, 11);
...
// Running STAPLE Tracker
float STAPLE_TRACKER::STAPLE_tracker(cv::Mat frame_in, cv::Mat template_in, cv::Rect template_roi, cv::Rect* Staple_result, bool detector_flag, bool* occl_flag )
{
    //printf("[%s][%s][%4.4d] \n", __FILE__, __func__, __LINE__);
    bool show_visualization = false;
    cv::Mat frame_in_clone;
    cv::Mat response_mat;
    std::vector<cv::Rect_<float>> result_rects;
    float response_score_max = 0.0;
    float staple_PSR = 0.0;

    frame_in_clone = frame_in.clone();

    if (detector_flag == true) 
    {
        tracker_staple_initialize(frame_in_clone, template_roi);
        tracker_staple_train(frame_in_clone, true);

        *Staple_result = template_roi;
    }
    else 
    {
        *Staple_result = tracker_staple_update(frame_in_clone, &response_score_max, &response_mat);     
        staple_PSR = ComputePSR(response_mat);

        if (staple_PSR > PSR_min_thresh) // Possible state of Occlusion
        {
            *occl_flag = false;
            tracker_staple_train(frame_in_clone, false);
        }
        else
        {
            *occl_flag = true;            
    ...
    result_rects.push_back(template_roi);

    if (show_visualization) 
    {
        cv::putText(frame_in_clone, std::to_string(0 + 1), cv::Point(20, 40), 6, 1,
            cv::Scalar(0, 255, 255), 2);
        //cv::rectangle(image, groundtruth_rect[frame], cv::Scalar(0, 255, 0), 2);
        cv::rectangle(frame_in_clone, *Staple_result, cv::Scalar(0, 128, 255), 2);
        //cv::imshow("STAPLE", frame_in_clone);
    }
           
    return response_score_max;
}
