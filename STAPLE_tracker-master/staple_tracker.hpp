...
struct staple_cfg
{
    ...
    double merge_factor = 0.0; //0.3 default          // fixed interpolation factor - how to linearly combine the two responses
    const char * merge_method = "const_factor";
    bool den_per_channel = false;

    // scale related
    bool scale_adaptation = false;
...
public:
    ...
    cv::Rect tracker_staple_update(const cv::Mat &im, float* response_score_max, cv::Mat* response_matrix);
    float STAPLE_tracker(cv::Mat frame_in, cv::Mat template_in, cv::Rect template_roi, cv::Rect* Staple_result, bool detector_flag, bool* occl_flag);
    void DFT_CGPU(cv::Mat img, int flag_dft);
